...
class LightningEffect(EffectBase):
    ...
    def editor_settings_schema(self):
        return [
            ...
            {"key": "spread", "label": "Arc Spread", "type": "slider", "min": 2, "max": 60, "step": 1, "default": 13},
    ...
    def render(self, ctx: EffectContext) -> List[Tuple[int, int, int]]:
        ...
        spread = max(2.0, min(60.0, float(ctx.effect_state.get("spread", 13.0))))
        ...
        if random.random() < spawn_chance:
            self._spawn_cluster(ctx.led_count, spread, branching, activity)

        if self._burst_remaining > 0:
            self._burst_delay_ms -= dt_ms
            if self._burst_delay_ms <= 0:
                self._spawn_cluster(ctx.led_count, spread * 0.9, branching * 0.8, activity * 0.8)
                self._burst_remaining -= 1
                self._burst_delay_ms = random.randint(40, 110)
        ...
        for i in range(ctx.led_count):
            ...
            power = (self._energy[i] ** 1.10) * brightness
            r = int(170 * power)
            g = int(205 * power)
            b = int(255 * power)
    ...
    def _spawn_cluster(self, led_count: int, spread: float, branching: float, activity: float) -> None:
        center = random.randint(0, max(0, led_count - 1))
        peak = 0.18 + (activity * 0.32) + random.uniform(0.02, 0.14)
        radius = max(2.0, spread * random.uniform(0.72, 1.18))
        duration = random.randint(140, 360)
        self._pulses.append(
            {
                "center": center,
                "peak": min(0.82, peak),
                ...
                "age_ms": 0,
                "flicker_hz": random.uniform(10.0, 18.0),
        ...
        for _ in range(branch_count):
            offset = int(random.uniform(-spread * 1.8, spread * 1.8))
            self._pulses.append(
                {
                    ...
                    "duration_ms": random.randint(90, 240),
                    "age_ms": 0,
                    "flicker_hz": random.uniform(12.0, 22.0),
                    "phase": random.uniform(0.0, math.pi * 2.0),
                }
            )
        ...
        if random.random() < (0.18 + (activity * 0.45)):
            ...
            self._burst_delay_ms = random.randint(30, 90)
